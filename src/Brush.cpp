...
void Brush::Paint(Texture *texture) {
    ...
    glDispatchCompute(texture->Width() / _program_work_group_size.x, texture->Height() / _program_work_group_size.y, 1);
...
void Brush::SetColor(glm::vec4 color) {
    _brush_data.color = color;
}
void Brush::SetPosition(glm::vec2 position) {
    _brush_data.position = position;
}
void Brush::SetPressure(float pressure) {
    _brush_data.pressure = pressure;
}
void Brush::SetOrientation(float orientation) {
    _brush_data.orientation = orientation;
}
void Brush::SetTilt(float tilt) {
    _brush_data.tilt = tilt;
}
void Brush::SetRotation(float rotation) {
    _brush_data.rotation = rotation;
}

glm::vec4 Brush::GetColor() {
    return _brush_data.color;
}
glm::vec2 Brush::GetPosition() {
    return _brush_data.position;
}
float Brush::GetPressure() {
    return _brush_data.pressure;
}
float Brush::GetOrientation() {
    return _brush_data.orientation;
}
float Brush::GetTilt() {
    return _brush_data.tilt;
}
float Brush::GetRotation() {
    return _brush_data.rotation;
}